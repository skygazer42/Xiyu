...
from .dataclasses import CTCToken
...
def decode_ctc(outputs: np.ndarray, id2token: Dict[int, str]) -> Tuple[str, List[CTCToken]]:
    ...
    for token_id, start in collapsed:
        ...
        results.append(CTCToken(text=token_text, start=t_start))

    full_text = "".join([r.text for r in results])
    return full_text, results


def align_timestamps(ctc_results: List[CTCToken], llm_text: str) -> List[dict]:
    ...
    def get_interpolated_start(target_idx):
        ...
        if prev_a and next_a:
            ...
            return prev_a[1] + 0.05
        elif next_a:
            return max(0, next_a[1] - 0.05)
        else:
            return 0.0
